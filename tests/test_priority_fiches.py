...
def test_common_use_cases_contains_bar_th_179():
    priority = json.loads((ROOT / "data/indexes/common_use_cases_index.json").read_text(encoding="utf-8"))
    codes = {item["code"] for item in priority}
    assert "BAR-TH-179" in codes
    assert "RES-CH-106" in codes


def test_common_use_cases_excludes_non_building_sectors_without_whitelist():
    priority = json.loads((ROOT / "data/indexes/common_use_cases_index.json").read_text(encoding="utf-8"))
